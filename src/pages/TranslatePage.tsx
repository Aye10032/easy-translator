...
import {
	...
	translationToneLabels,
	translationTones,
} from "../ai/tones";
import type {
	ModelProfile,
	TranslationTone,
	TranslationToneSettings,
} from "../ai/types";
...
import {
	...
	loadModelProfiles,
	loadSettings,
	loadTranslationToneSettings,
	setActiveModel,
	setActiveTranslationTone,
} from "../lib/settings";
...
const AUTOMATIC_LANGUAGE = "自动检测";

function getAutomaticTargetLanguage(text: string) {
	const containsJapaneseKana = /[\p{Script=Hiragana}\p{Script=Katakana}]/u.test(
		text,
	);
	const containsChineseCharacters = /\p{Script=Han}/u.test(text);
	return containsChineseCharacters && !containsJapaneseKana ? "英语" : "中文";
}
...
export function TranslatePage() {
	...
	const [toneSettings, setToneSettings] = useState<TranslationToneSettings>(
		defaultTranslationToneSettings,
	);
	const [sourceLanguage, setSourceLanguage] = useState(AUTOMATIC_LANGUAGE);
	...
	useEffect(() => {
		void Promise.all([loadModelProfiles(), loadTranslationToneSettings()]).then(
			([state, savedToneSettings]) => {
				setProfiles(state.profiles);
				setSettings(
					state.profiles.find(
						(profile) => profile.id === state.activeModelId,
					) ?? state.profiles[0],
				);
	...
	useEffect(() => {
		if (sourceLanguage !== AUTOMATIC_LANGUAGE || !source.trim()) return;
		setTargetLanguage(getAutomaticTargetLanguage(source));
	...
	useEffect(() => {
		...
		void getTranslationHistoryItem(historyId)
			...
			.catch((cause) =>
				toast.error("读取历史记录失败", { description: String(cause) }),
			)
	...
	function swapLanguages() {
		if (sourceLanguage === AUTOMATIC_LANGUAGE) return;
		...
		try {
			...
			if (!controller.signal.aborted && content.trim()) {
				try {
					...
					toast.warning("译文已生成，但历史记录保存失败", {
						description: String(historyError),
					});
				}
			}
		} catch (cause) {
					...
					<Tooltip>
						<TooltipTrigger asChild>
							<Button
								...
								disabled={sourceLanguage === AUTOMATIC_LANGUAGE}
								aria-label="交换语言"
							>
				...
				<div className="translation-panels">
					<article className="translation-panel source-panel">
						<div className="panel-heading">
							<span>原文</span>
							{source && (
								<Button variant="ghost" size="sm" onClick={() => setSource("")}>
									<X size={14} />
									清空
								</Button>
							)}
					...
					<article className="translation-panel result-panel">
						...
						{translation ? (
							...
							<div className="panel-state">
								<div className="state-icon active">
									<LoaderCircle className="spin" size={22} />
								</div>
								<strong>正在连接模型</strong>
								<p>译文会实时出现在这里</p>
							</div>
						) : (
							<div className="panel-state">
					...
					<div className="workbench-actions">
						<div className="footer-model-tools">
							<Select
								...
								onValueChange={(value) =>
									void changeTone(value as TranslationTone)
								}
							...
							>
								...
								<SelectContent>
									{translationTones.map((tone) => (
										<SelectItem value={tone} key={tone}>
											{translationToneLabels[tone]}
										</SelectItem>
									))}
								</SelectContent>
							</Select>
							<Select
								...
								disabled={translating}
							>
								<SelectTrigger className="model-select" aria-label="选择模型">
									<SelectValue />
								</SelectTrigger>
								<SelectContent>
							...
							<Tooltip>
								<TooltipTrigger asChild>
									<Button asChild variant="secondary" size="icon">
										<Link to="/settings" aria-label="管理模型">
											<Settings2 size={17} />
										</Link>
									</Button>
								</TooltipTrigger>
